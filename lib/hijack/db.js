/* global MongoInternals, _, Mongo, Meteor, Random, OptimizedApply, MeteorX, Kadira, HaveAsyncCallback */
...
hijackDBOps = function hijackDBOps() {
  ...
  mongoConnectionProto.originalRawCollection = mongoConnectionProto.rawCollection;
  mongoConnectionProto.rawCollection = function (collectionName) {
    var Collection = this.originalRawCollection(collectionName);
    [ 'find', 'update', 'updateOne', 'remove', 'insert' ].forEach(function (func) {
      var originalFunc = Collection[func];
      Collection[func] = function (selector, mod, options) {
        // console.log(`calling ${collectionName}.${func}`);
        var payload = {
          coll: collectionName,
          func: func
        };
        ...
        if (func == 'insert') {
          ...
          payload.modifier = JSON.stringify(mod);
        } else if (func == 'updateOne' || func == 'update') {
          payload.selector = JSON.stringify(selector);
          payload.modifier = JSON.stringify(mod);
        } else {
          ...
          if (eventId) {
            if (ret instanceof Promise) {
              ret.then(function () {
                ...
                if (eventId) {
                  Kadira.tracer.eventEnd(kadiraInfo.trace, eventId, { err: reason.message });
                }
              });
            } else {
              Kadira.tracer.eventEnd(kadiraInfo.trace, eventId, endOptions);
  ...
  [ 'find', 'update', 'remove', 'insert', '_ensureIndex', '_dropIndex' ].forEach(function (func) {
    ...
    mongoConnectionProto[func] = function (collName, selector, _mod, options) {
      var payload = {
        coll: collName,
        func: func
      };
